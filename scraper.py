...
def scrap_item(url):
    ...
    special_prices = json.loads(pq("div.ovabrw__product_calendar").attr("data-special-time"))
    item["special_prices"] = {
        PyQuery(k).text(): tuple(date.fromtimestamp(int(timestamp)).strftime("%Y-%m-%d") for timestamp in v) for (k, v) in special_prices.items()
    }
    try:
...
def get_apartments_urls():
    ...
    response = requests.post('https://www.sanmatiaspropiedades.com.ar/wp-admin/admin-ajax.php', headers=headers, data=data)

    return sorted({e.attrib["href"].partition("?")[0] for e in PyQuery(response.json()["result"])("a")})
